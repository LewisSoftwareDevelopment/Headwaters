...
$layout->blocks["top"][] = "export";$page_layouts["bankers_IBC_Closed_export"] = $layout;
...
$xt->display("bankers_IBC_Closed_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=bankers_IBC_Closed.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankers_IBC_Closed.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankers_IBC_Closed.xml");
	...
	{
		$values = array();
			$values["Rows"] = $pageObject->showDBValue("Rows", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankers_IBC_Closed.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["Rows"] = $pageObject->getViewControl("Rows")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Rows"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Rows").'</td>';	
