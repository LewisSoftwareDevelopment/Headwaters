...
include("include/Changes_Since_Last_Month_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["Changes_Since_Last_Month_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Changes_Since_Last_Month_search.htm";
...
$params['shortTableName'] = 'Changes_Since_Last_Month';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Deed"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Deed")] = GetTableURL($pageObject->pSet->getLookupTable("Deed"));
	
	$pageObject->fillFieldToolTips("Deed");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Deed");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Deed_label","<label for=\"".GetInputElementId("Deed", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Deed_label", true);
	
	$xt->assign("Deed_fieldblock", true);
	$xt->assignbyref("Deed_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Deed_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Deed_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Deed", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Deed");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Deed", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Deed", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("LM"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("LM")] = GetTableURL($pageObject->pSet->getLookupTable("LM"));
	
	$pageObject->fillFieldToolTips("LM");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("LM");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("LM_label","<label for=\"".GetInputElementId("LM", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("LM_label", true);
	
	$xt->assign("LM_fieldblock", true);
	$xt->assignbyref("LM_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("LM_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("LM_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_LM", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("LM");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"LM", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"LM", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
