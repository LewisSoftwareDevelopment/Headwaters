...
add_nocache_headers();
include("include/bprevstore_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["bprevstore_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "bprevstore_inline_add.htm";
else
	$templatefile = "bprevstore_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: bprevstore_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'bprevstore_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'bprevstore_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: bprevstore_".$pageObject->getPageType().".php");
...
////////////////////////////////////////////
		...
		$showRawValues["y1"] = substr($data["y1"],0,100);
		$showRawValues["y2"] = substr($data["y2"],0,100);
		$showRawValues["y3"] = substr($data["y3"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "bprevstore";
		$options['firstTime'] = 1;
