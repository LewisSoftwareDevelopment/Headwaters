...
$layout->blocks["top"][] = "export";$page_layouts["Revenue_Estimates1_export"] = $layout;
...
$xt->display("Revenue_Estimates1_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Revenue_Estimates1.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Revenue_Estimates1.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Revenue_Estimates1.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Pipeline"] = $pageObject->showDBValue("Pipeline", $row);
			$values["This Year Buget Gross"] = $pageObject->showDBValue("This Year Buget Gross", $row);
			$values["This Year Budget Net"] = $pageObject->showDBValue("This Year Budget Net", $row);
			$values["YTD Net Actual"] = $pageObject->showDBValue("YTD Net Actual", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Revenue_Estimates1.csv");
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"This Year Buget Gross\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"This Year Budget Net\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"YTD Net Actual\"";
	echo $outstr;
	echo "\r\n";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Pipeline"] = $pageObject->getViewControl("Pipeline")->showDBValue($row, "");
			$values["This Year Buget Gross"] = $pageObject->getViewControl("This Year Buget Gross")->showDBValue($row, "");
			$values["This Year Budget Net"] = $pageObject->getViewControl("This Year Budget Net")->showDBValue($row, "");
			$values["YTD Net Actual"] = $pageObject->getViewControl("YTD Net Actual")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Pipeline"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["This Year Buget Gross"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["This Year Budget Net"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["YTD Net Actual"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Pipeline").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("This Year Buget Gross").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("This Year Budget Net").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("YTD Net Actual").'</td>';	
	}
	else
	{
		echo "<td>"."Company"."</td>";
		...
		echo "<td>"."Pipeline"."</td>";
		echo "<td>"."This Year Buget Gross"."</td>";
		echo "<td>"."This Year Budget Net"."</td>";
		echo "<td>"."YTD Net Actual"."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Pipeline"]);
					else
						echo $values["Pipeline"];
			echo '</td>';
							echo '<td>';
			
									echo $values["This Year Buget Gross"];
			echo '</td>';
							echo '<td>';
			
									echo $values["This Year Budget Net"];
			echo '</td>';
							echo '<td>';
			
									echo $values["YTD Net Actual"];
			echo '</td>';
			echo "</tr>";
		}
