...
$layout->blocks["top"][] = "export";$page_layouts["bankers_Report3_export"] = $layout;
...
$xt->display("bankers_Report3_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=bankers_Report3.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankers_Report3.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankers_Report3.xml");
	...
	{
		$values = array();
			$values["Year (CURDATE())-3"] = $pageObject->showDBValue("Year (CURDATE())-3", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankers_Report3.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["Year (CURDATE())-3"] = $pageObject->getViewControl("Year (CURDATE())-3")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Year (CURDATE())-3"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Year (CURDATE())-3").'</td>';	
	}
	else
	{
		echo "<td>"."Year (CURDATE())-3"."</td>";
	}
	echo "</tr>";
