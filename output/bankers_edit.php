...
$layout->blocks["top"][] = "details";$page_layouts["bankers_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "bankers_inline_edit.htm" : "bankers_edit.htm";
...
//	processing Budget Year - begin
	$condition = 1;

	if($condition)
	{
		$control_Budget_Year = $pageObject->getControl("Budget Year", $id);
		$control_Budget_Year->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing YTD+Last - begin
	$condition = 1;

	if($condition)
	{
		$control_YTD_Last = $pageObject->getControl("YTD+Last", $id);
		$control_YTD_Last->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing YTD+Last - end
//	processing Last Name - begin
	$condition = 1;

	if($condition)
	{
		$control_Last_Name = $pageObject->getControl("Last Name", $id);
		$control_Last_Name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Last Name - end
//	processing First Name - begin
	$condition = 1;

	if($condition)
	{
		$control_First_Name = $pageObject->getControl("First Name", $id);
		$control_First_Name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: bankers_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["Start Date"] = $evalues["Start Date"];
	$data["Budget Year"] = $evalues["Budget Year"];
	$data["Active"] = $evalues["Active"];
	$data["YTD+Last"] = $evalues["YTD+Last"];
	$data["Last Name"] = $evalues["Last Name"];
	$data["First Name"] = $evalues["First Name"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Budget_Year_label","<label for=\"".GetInputElementId("Budget Year", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("YTD_Last_label","<label for=\"".GetInputElementId("YTD+Last", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Last_Name_label","<label for=\"".GetInputElementId("Last Name", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("First_Name_label","<label for=\"".GetInputElementId("First Name", $id, PAGE_EDIT)."\">","</label>");
...
foreach($pageObject->editFields as $fName)
{
	...
	{
		$_SESSION[$strTableName."_"."Active"."_rte"] = true;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='bankers_view.php?".$viewlink."'\"");
