...
add_nocache_headers();
include("include/bpengagements_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["bpengagements_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "bpengagements_inline_add.htm";
else
	$templatefile = "bpengagements_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: bpengagements_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'bpengagements_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'bpengagements_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: bpengagements_".$pageObject->getPageType().".php");
...
////////////////////////////////////////////
		...
		$showRawValues["IBC"] = substr($data["IBC"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "bpengagements";
		$options['firstTime'] = 1;
