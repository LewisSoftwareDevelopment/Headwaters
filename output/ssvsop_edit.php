...
$layout->blocks["top"][] = "details";$page_layouts["ssvsop_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "ssvsop_inline_edit.htm" : "ssvsop_edit.htm";
...
//	processing Jan - begin
	$condition = 1;

	if($condition)
	{
		$control_Jan = $pageObject->getControl("Jan", $id);
		$control_Jan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Jan - end
//	processing Feb - begin
	$condition = 1;

	if($condition)
	{
		$control_Feb = $pageObject->getControl("Feb", $id);
		$control_Feb->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Feb - end
//	processing Mar - begin
	$condition = 1;

	if($condition)
	{
		$control_Mar = $pageObject->getControl("Mar", $id);
		$control_Mar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Mar - end
//	processing April - begin
	$condition = 1;

	if($condition)
	{
		$control_April = $pageObject->getControl("April", $id);
		$control_April->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing April - end
//	processing May - begin
	$condition = 1;

	if($condition)
	{
		$control_May = $pageObject->getControl("May", $id);
		$control_May->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing May - end
//	processing June - begin
	$condition = 1;

	if($condition)
	{
		$control_June = $pageObject->getControl("June", $id);
		$control_June->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing June - end
//	processing July - begin
	$condition = 1;

	if($condition)
	{
		$control_July = $pageObject->getControl("July", $id);
		$control_July->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing July - end
//	processing Aug - begin
	$condition = 1;

	if($condition)
	{
		$control_Aug = $pageObject->getControl("Aug", $id);
		$control_Aug->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Aug - end
//	processing Sept - begin
	$condition = 1;

	if($condition)
	{
		$control_Sept = $pageObject->getControl("Sept", $id);
		$control_Sept->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Sept - end
//	processing Oct - begin
	$condition = 1;

	if($condition)
	{
		$control_Oct = $pageObject->getControl("Oct", $id);
		$control_Oct->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Oct - end
//	processing Nov - begin
	$condition = 1;

	if($condition)
	{
		$control_Nov = $pageObject->getControl("Nov", $id);
		$control_Nov->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Nov - end
//	processing Dec - begin
	$condition = 1;

	if($condition)
	{
		$control_Dec = $pageObject->getControl("Dec", $id);
		$control_Dec->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing SS - begin
	...
	if($condition)
	{
		$control_SS = $pageObject->getControl("SS", $id);
		$control_SS->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing SS - end
//	processing Live - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Live = $pageObject->getControl("Live", $id);
		$control_Live->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Live - end
//	processing ssovel - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_ssovel = $pageObject->getControl("ssovel", $id);
		$control_ssovel->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ssovel - end
	...
	//	do event
		...
		{		
			...
			//	processing Jan - begin
				...
				if($condition)
				{
					$control_Jan->afterSuccessfulSave();
				}
	//	processing Jan - end
			//	processing Feb - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Feb->afterSuccessfulSave();
				}
	//	processing Feb - end
			//	processing Mar - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Mar->afterSuccessfulSave();
				}
	//	processing Mar - end
			//	processing April - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_April->afterSuccessfulSave();
				}
	//	processing April - end
			//	processing May - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_May->afterSuccessfulSave();
				}
	//	processing May - end
			//	processing June - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_June->afterSuccessfulSave();
				}
	//	processing June - end
			//	processing July - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_July->afterSuccessfulSave();
				}
	//	processing July - end
			//	processing Aug - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Aug->afterSuccessfulSave();
				}
	//	processing Aug - end
			//	processing Sept - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Sept->afterSuccessfulSave();
				}
	//	processing Sept - end
			//	processing Oct - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Oct->afterSuccessfulSave();
				}
	//	processing Oct - end
			//	processing Nov - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Nov->afterSuccessfulSave();
				}
	//	processing Nov - end
			//	processing Dec - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Dec->afterSuccessfulSave();
				}
	//	processing Dec - end
			//	processing SS - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_SS->afterSuccessfulSave();
				}
	//	processing SS - end
			//	processing Live - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_Live->afterSuccessfulSave();
				}
	//	processing Live - end
			//	processing ssovel - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_ssovel->afterSuccessfulSave();
				}
	//	processing ssovel - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: ssvsop_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: ssvsop_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Jan"] = $evalues["Jan"];
	$data["Feb"] = $evalues["Feb"];
	$data["Mar"] = $evalues["Mar"];
	$data["April"] = $evalues["April"];
	$data["May"] = $evalues["May"];
	$data["June"] = $evalues["June"];
	$data["July"] = $evalues["July"];
	$data["Aug"] = $evalues["Aug"];
	$data["Sept"] = $evalues["Sept"];
	$data["Oct"] = $evalues["Oct"];
	$data["Nov"] = $evalues["Nov"];
	$data["Dec"] = $evalues["Dec"];
	$data["SS"] = $evalues["SS"];
	$data["Live"] = $evalues["Live"];
	$data["ssovel"] = $evalues["ssovel"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Jan_label","<label for=\"".GetInputElementId("Jan", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Feb_label","<label for=\"".GetInputElementId("Feb", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Mar_label","<label for=\"".GetInputElementId("Mar", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("April_label","<label for=\"".GetInputElementId("April", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("May_label","<label for=\"".GetInputElementId("May", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("June_label","<label for=\"".GetInputElementId("June", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("July_label","<label for=\"".GetInputElementId("July", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Aug_label","<label for=\"".GetInputElementId("Aug", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Sept"))
		$xt->assign("Sept_fieldblock",true);
	else
		$xt->assign("Sept_tabfieldblock",true);
	$xt->assign("Sept_label",true);
	if(isEnableSection508())
		$xt->assign_section("Sept_label","<label for=\"".GetInputElementId("Sept", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Oct_label","<label for=\"".GetInputElementId("Oct", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Nov_label","<label for=\"".GetInputElementId("Nov", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Dec_label","<label for=\"".GetInputElementId("Dec", $id, PAGE_EDIT)."\">","</label>");
...
//	Jan - 
	...
	$showValues["Jan"] = $value;
	$showFields[] = "Jan";
		$showRawValues["Jan"] = substr($data["Jan"],0,100);

//	Feb - 
	$value = $pageObject->showDBValue("Feb", $data, $keylink);
	$showValues["Feb"] = $value;
	$showFields[] = "Feb";
		$showRawValues["Feb"] = substr($data["Feb"],0,100);

//	Mar - 
	$value = $pageObject->showDBValue("Mar", $data, $keylink);
	$showValues["Mar"] = $value;
	$showFields[] = "Mar";
		$showRawValues["Mar"] = substr($data["Mar"],0,100);

//	April - 
	$value = $pageObject->showDBValue("April", $data, $keylink);
	$showValues["April"] = $value;
	$showFields[] = "April";
		$showRawValues["April"] = substr($data["April"],0,100);

//	May - 
	$value = $pageObject->showDBValue("May", $data, $keylink);
	$showValues["May"] = $value;
	$showFields[] = "May";
		$showRawValues["May"] = substr($data["May"],0,100);

//	June - 
	$value = $pageObject->showDBValue("June", $data, $keylink);
	$showValues["June"] = $value;
	$showFields[] = "June";
		$showRawValues["June"] = substr($data["June"],0,100);

//	July - 
	$value = $pageObject->showDBValue("July", $data, $keylink);
	$showValues["July"] = $value;
	$showFields[] = "July";
		$showRawValues["July"] = substr($data["July"],0,100);

//	Aug - 
	$value = $pageObject->showDBValue("Aug", $data, $keylink);
	$showValues["Aug"] = $value;
	$showFields[] = "Aug";
		$showRawValues["Aug"] = substr($data["Aug"],0,100);

//	Sept - 
	$value = $pageObject->showDBValue("Sept", $data, $keylink);
	$showValues["Sept"] = $value;
	$showFields[] = "Sept";
		$showRawValues["Sept"] = substr($data["Sept"],0,100);

//	Oct - 
	$value = $pageObject->showDBValue("Oct", $data, $keylink);
	$showValues["Oct"] = $value;
	$showFields[] = "Oct";
		$showRawValues["Oct"] = substr($data["Oct"],0,100);

//	Nov - 
	$value = $pageObject->showDBValue("Nov", $data, $keylink);
	$showValues["Nov"] = $value;
	$showFields[] = "Nov";
		$showRawValues["Nov"] = substr($data["Nov"],0,100);

//	Dec - 
	$value = $pageObject->showDBValue("Dec", $data, $keylink);
	$showValues["Dec"] = $value;
	$showFields[] = "Dec";
		$showRawValues["Dec"] = substr($data["Dec"],0,100);

//	SS - 
	$value = $pageObject->showDBValue("SS", $data, $keylink);
	$showValues["SS"] = $value;
	$showFields[] = "SS";
		$showRawValues["SS"] = substr($data["SS"],0,100);

//	Live - 
	$value = $pageObject->showDBValue("Live", $data, $keylink);
	$showValues["Live"] = $value;
	$showFields[] = "Live";
		$showRawValues["Live"] = substr($data["Live"],0,100);

//	ssovel - Number
	$value = $pageObject->showDBValue("ssovel", $data, $keylink);
	$showValues["ssovel"] = $value;
	$showFields[] = "ssovel";
		$showRawValues["ssovel"] = substr($data["ssovel"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='ssvsop_view.php?".$viewlink."'\"");
