...
$layout->blocks["top"][] = "details";$page_layouts["inmarketloi_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "inmarketloi_inline_add.htm";
else
	$templatefile = "inmarketloi_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: inmarketloi_add.php');
...
//	processing Aug - end
//	processing Sep - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Sep = $pageObject->getControl("Sep", $id);
		$control_Sep->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Sep - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'inmarketloi_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'inmarketloi_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: inmarketloi_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Sep_label","<label for=\"".GetInputElementId("Sep", $id, PAGE_ADD)."\">","</label>");
