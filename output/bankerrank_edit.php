...
include("include/bankerrank_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["bankerrank_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "bankerrank_inline_edit.htm" : "bankerrank_edit.htm";
...
//	processing Name - end
//	processing Rank - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Rank = $pageObject->getControl("Rank", $id);
		$control_Rank->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Rank - end
	...
	//	processing Name - end
			//	processing Rank - begin
				...
				if($condition)
				{
					$control_Rank->afterSuccessfulSave();
				}
	//	processing Rank - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: bankerrank_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: bankerrank_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Rank"))
		$xt->assign("Rank_fieldblock",true);
	else
		$xt->assign("Rank_tabfieldblock",true);
	$xt->assign("Rank_label",true);
	if(isEnableSection508())
		$xt->assign_section("Rank_label","<label for=\"".GetInputElementId("Rank", $id, PAGE_EDIT)."\">","</label>");
...
//	Rank - 
	$value = $pageObject->showDBValue("Rank", $data, $keylink);
	$showValues["Rank"] = $value;
	$showFields[] = "Rank";
		$showRawValues["Rank"] = substr($data["Rank"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "bankerrank";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='bankerrank_view.php?".$viewlink."'\"");
