...
{	
	...
	*/
	...
	{	
		...
		$userheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby=Auser\"";
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			foreach($this->groups as $idx => $g)
			{
				...
				if($_SESSION["UserID"] != $data["user"] || $g[0] != -1)
				{
			...
			$row["displayusername"] = htmlspecialchars($data["Name"]); //htmlspecialchars($data["user"]);
			$row["user"] = $data["user"];
			$usercheckbox_attrs.=" name=\"user_".count($this->users)
				."\" id=\"".count($this->users)."\"";
			
			$this->users[] = htmlspecialchars($data["user"]);
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "user" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) =="Name"){
