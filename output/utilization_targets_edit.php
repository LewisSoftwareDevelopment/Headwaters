...
$layout->blocks["top"][] = "details";$page_layouts["utilization_targets_edit"] = $layout;
...
$templatefile = "utilization_targets_edit.htm";
...
//	processing ID - end
//	processing IBC per MD - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_IBC_per_MD = $pageObject->getControl("IBC per MD", $id);
		$control_IBC_per_MD->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing IBC per MD - end
//	processing IBC Total MD - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_IBC_Total_MD = $pageObject->getControl("IBC Total MD", $id);
		$control_IBC_Total_MD->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing IBC Total MD - end
//	processing IBC YTD Target - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_IBC_YTD_Target = $pageObject->getControl("IBC YTD Target", $id);
		$control_IBC_YTD_Target->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing IBC YTD Target - end
//	processing EL per MD - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_EL_per_MD = $pageObject->getControl("EL per MD", $id);
		$control_EL_per_MD->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing EL per MD - end
//	processing EL Total MD - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_EL_Total_MD = $pageObject->getControl("EL Total MD", $id);
		$control_EL_Total_MD->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing EL Total MD - end
//	processing EL YTD Target - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_EL_YTD_Target = $pageObject->getControl("EL YTD Target", $id);
		$control_EL_YTD_Target->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing EL YTD Target - end
//	processing Closing per MD - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Closing_per_MD = $pageObject->getControl("Closing per MD", $id);
		$control_Closing_per_MD->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Closing per MD - end
//	processing Closing Total MD - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Closing_Total_MD = $pageObject->getControl("Closing Total MD", $id);
		$control_Closing_Total_MD->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Closing Total MD - end
//	processing Closing YTD MD - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Closing_YTD_MD = $pageObject->getControl("Closing YTD MD", $id);
		$control_Closing_YTD_MD->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Closing YTD MD - end
	...
	//	processing ID - end
			//	processing IBC per MD - begin
				...
				if($condition)
				{
					$control_IBC_per_MD->afterSuccessfulSave();
				}
	//	processing IBC per MD - end
			//	processing IBC Total MD - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_IBC_Total_MD->afterSuccessfulSave();
				}
	//	processing IBC Total MD - end
			//	processing IBC YTD Target - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_IBC_YTD_Target->afterSuccessfulSave();
				}
	//	processing IBC YTD Target - end
			//	processing EL per MD - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_EL_per_MD->afterSuccessfulSave();
				}
	//	processing EL per MD - end
			//	processing EL Total MD - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_EL_Total_MD->afterSuccessfulSave();
				}
	//	processing EL Total MD - end
			//	processing EL YTD Target - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_EL_YTD_Target->afterSuccessfulSave();
				}
	//	processing EL YTD Target - end
			//	processing Closing per MD - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_Closing_per_MD->afterSuccessfulSave();
				}
	//	processing Closing per MD - end
			//	processing Closing Total MD - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_Closing_Total_MD->afterSuccessfulSave();
				}
	//	processing Closing Total MD - end
			//	processing Closing YTD MD - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_Closing_YTD_MD->afterSuccessfulSave();
				}
	//	processing Closing YTD MD - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: utilization_targets_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: utilization_targets_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
{
	$data["ID"] = $evalues["ID"];
	$data["IBC per MD"] = $evalues["IBC per MD"];
	$data["IBC Total MD"] = $evalues["IBC Total MD"];
	$data["IBC YTD Target"] = $evalues["IBC YTD Target"];
	$data["EL per MD"] = $evalues["EL per MD"];
	$data["EL Total MD"] = $evalues["EL Total MD"];
	$data["EL YTD Target"] = $evalues["EL YTD Target"];
	$data["Closing per MD"] = $evalues["Closing per MD"];
	$data["Closing Total MD"] = $evalues["Closing Total MD"];
	$data["Closing YTD MD"] = $evalues["Closing YTD MD"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("IBC per MD"))
		$xt->assign("IBC_per_MD_fieldblock",true);
	else
		$xt->assign("IBC_per_MD_tabfieldblock",true);
	$xt->assign("IBC_per_MD_label",true);
	if(isEnableSection508())
		$xt->assign_section("IBC_per_MD_label","<label for=\"".GetInputElementId("IBC per MD", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("IBC Total MD"))
		$xt->assign("IBC_Total_MD_fieldblock",true);
	else
		$xt->assign("IBC_Total_MD_tabfieldblock",true);
	$xt->assign("IBC_Total_MD_label",true);
	if(isEnableSection508())
		$xt->assign_section("IBC_Total_MD_label","<label for=\"".GetInputElementId("IBC Total MD", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("IBC YTD Target"))
		$xt->assign("IBC_YTD_Target_fieldblock",true);
	else
		$xt->assign("IBC_YTD_Target_tabfieldblock",true);
	$xt->assign("IBC_YTD_Target_label",true);
	if(isEnableSection508())
		$xt->assign_section("IBC_YTD_Target_label","<label for=\"".GetInputElementId("IBC YTD Target", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("EL per MD"))
		$xt->assign("EL_per_MD_fieldblock",true);
	else
		$xt->assign("EL_per_MD_tabfieldblock",true);
	$xt->assign("EL_per_MD_label",true);
	if(isEnableSection508())
		$xt->assign_section("EL_per_MD_label","<label for=\"".GetInputElementId("EL per MD", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("EL Total MD"))
		$xt->assign("EL_Total_MD_fieldblock",true);
	else
		$xt->assign("EL_Total_MD_tabfieldblock",true);
	$xt->assign("EL_Total_MD_label",true);
	if(isEnableSection508())
		$xt->assign_section("EL_Total_MD_label","<label for=\"".GetInputElementId("EL Total MD", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("EL YTD Target"))
		$xt->assign("EL_YTD_Target_fieldblock",true);
	else
		$xt->assign("EL_YTD_Target_tabfieldblock",true);
	$xt->assign("EL_YTD_Target_label",true);
	if(isEnableSection508())
		$xt->assign_section("EL_YTD_Target_label","<label for=\"".GetInputElementId("EL YTD Target", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Closing per MD"))
		$xt->assign("Closing_per_MD_fieldblock",true);
	else
		$xt->assign("Closing_per_MD_tabfieldblock",true);
	$xt->assign("Closing_per_MD_label",true);
	if(isEnableSection508())
		$xt->assign_section("Closing_per_MD_label","<label for=\"".GetInputElementId("Closing per MD", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Closing Total MD"))
		$xt->assign("Closing_Total_MD_fieldblock",true);
	else
		$xt->assign("Closing_Total_MD_tabfieldblock",true);
	$xt->assign("Closing_Total_MD_label",true);
	if(isEnableSection508())
		$xt->assign_section("Closing_Total_MD_label","<label for=\"".GetInputElementId("Closing Total MD", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Closing YTD MD"))
		$xt->assign("Closing_YTD_MD_fieldblock",true);
	else
		$xt->assign("Closing_YTD_MD_tabfieldblock",true);
	$xt->assign("Closing_YTD_MD_label",true);
	if(isEnableSection508())
		$xt->assign_section("Closing_YTD_MD_label","<label for=\"".GetInputElementId("Closing YTD MD", $id, PAGE_EDIT)."\">","</label>");
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "utilization targets";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "utilization targets";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='utilization_targets_view.php?".$viewlink."'\"");
