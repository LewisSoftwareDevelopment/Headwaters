...
$layout->blocks["top"][] = "details";$page_layouts["bankerrank_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "bankerrank_inline_add.htm";
else
	$templatefile = "bankerrank_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: bankerrank_add.php');
...
//	processing Name - end
//	processing Rank - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Rank = $pageObject->getControl("Rank", $id);
		$control_Rank->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Rank - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'bankerrank_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'bankerrank_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: bankerrank_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Rank_label","<label for=\"".GetInputElementId("Rank", $id, PAGE_ADD)."\">","</label>");
