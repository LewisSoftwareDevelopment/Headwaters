...
add_nocache_headers();
include("include/fee_split_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["fee_split_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "fee_split_inline_add.htm";
else
	$templatefile = "fee_split_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: fee_split_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'fee_split_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'fee_split_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: fee_split_".$pageObject->getPageType().".php");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="company" && postvalue("field")=="Fee_To_1") 
	{
		$lookupFieldName = "Fee To-1";
		$lookupTable = "fee split";
		$linkFieldName = "Name";
		$dispfield = "Name";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Fee To-1");
	...
	if(postvalue("table")=="company" && postvalue("field")=="Fee_To_2") 
	{
		$lookupFieldName = "Fee To-2";
		$lookupTable = "fee split";
		$linkFieldName = "Name";
		$dispfield = "Name";
		// Table which contain lookup field!
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Fee To-2");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="company" && postvalue("field")=="Fee_To_3") 
	{
		$lookupFieldName = "Fee To-3";
		$lookupTable = "fee split";
		$linkFieldName = "Name";
		$dispfield = "Name";
		// Table which contain lookup field!
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Fee To-3");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="company" && postvalue("field")=="Fee_To_4") 
	{
		$lookupFieldName = "Fee To-4";
		$lookupTable = "fee split";
		$linkFieldName = "Name";
		$dispfield = "Name";
		// Table which contain lookup field!
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Fee To-4");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="company" && postvalue("field")=="Fee_To_5") 
	{
		$lookupFieldName = "Fee To-5";
		$lookupTable = "fee split";
		$linkFieldName = "Name";
		$dispfield = "Name";
		// Table which contain lookup field!
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Fee To-5");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
	...
	if(postvalue("table")=="company" && postvalue("field")=="Fee_To_6") 
	{
		$lookupFieldName = "Fee To-6";
		$lookupTable = "fee split";
		$linkFieldName = "Name";
		$dispfield = "Name";
		// Table which contain lookup field!
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Fee To-6");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
