...
include("include/bpengagmentstotal_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["bpengagmentstotal_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "bpengagmentstotal_inline_edit.htm" : "bpengagmentstotal_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: bpengagmentstotal_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: bpengagmentstotal_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	rows - 
	$value = $pageObject->showDBValue("rows", $data, $keylink);
	$showValues["rows"] = $value;
	$showFields[] = "rows";
		$showRawValues["rows"] = substr($data["rows"],0,100);

//	Banker - 
	$value = $pageObject->showDBValue("Banker", $data, $keylink);
	$showValues["Banker"] = $value;
	$showFields[] = "Banker";
		$showRawValues["Banker"] = substr($data["Banker"],0,100);

//	Engagements - 
	$value = $pageObject->showDBValue("Engagements", $data, $keylink);
	$showValues["Engagements"] = $value;
	$showFields[] = "Engagements";
		$showRawValues["Engagements"] = substr($data["Engagements"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "bpengagmentstotal";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "bpengagmentstotal";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='bpengagmentstotal_view.php?".$viewlink."'\"");
