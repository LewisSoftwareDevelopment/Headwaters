...
include("include/Live_Engagements_by_Deal_Slot_variables.php");
...
$layout->blocks["top"][] = "grid";$page_layouts["Live_Engagements_by_Deal_Slot_rprint"] = $layout;
...
$params["templatefile"] = "Live_Engagements_by_Deal_Slot_rprint.htm";
$params["xt"] = &$xt;
...
if (@$_REQUEST["format"]=="excel")
{
	...
	header("Content-Disposition: attachment;Filename=Live_Engagements_by_Deal_Slot.xls");
...
else if (@$_REQUEST["format"]=="word")
{
	...
	header("Content-Disposition: attachment;Filename=Live_Engagements_by_Deal_Slot.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
if(!$_SESSION[$sessionPrefix."_pagesize"])
	$_SESSION[$sessionPrefix."_pagesize"]=60;
...
if($cross_table)
{
	...
	$cross_array = array();
	$t="Live_Engagements_by_Deal_Slot";
...
else
{
	$rb = new Report(array($gQuery->HeadToSql(), $gQuery->FromToSql(), $strWhereClause, $gQuery->GroupByToSql(), $gQuery->HavingToSql())
		, $pageObject->pSet->GetTableData(".orderindexes"), $pageObject->searchClauseObj, $conn, $PageSize, 60, $params, $pageObject);
