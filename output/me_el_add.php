...
$layout->blocks["top"][] = "details";$page_layouts["me_el_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "me_el_inline_add.htm";
else
	$templatefile = "me_el_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing ELClient - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ELClient = $pageObject->getControl("ELClient", $id);
		$control_ELClient->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'me_el_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'me_el_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: me_el_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["ELClient"]=@$avalues["ELClient"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("ELClient_label","<label for=\"".GetInputElementId("ELClient", $id, PAGE_ADD)."\">","</label>");
