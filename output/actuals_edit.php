...
include("include/actuals_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["actuals_edit"] = $layout;
...
//	processing YTD Net Actual - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_YTD_Net_Actual = $pageObject->getControl("YTD Net Actual", $id);
		$control_YTD_Net_Actual->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing YTD Net Actual - end
//	processing This Year Buget Gross - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_This_Year_Buget_Gross = $pageObject->getControl("This Year Buget Gross", $id);
		$control_This_Year_Buget_Gross->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing This Year Buget Gross - end
//	processing This Year Budget Net - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_This_Year_Budget_Net = $pageObject->getControl("This Year Budget Net", $id);
		$control_This_Year_Budget_Net->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing This Year Budget Net - end
	...
	//	do event
		...
		{		
			...
			//	processing YTD Net Actual - begin
				...
				if($condition)
				{
					$control_YTD_Net_Actual->afterSuccessfulSave();
				}
	//	processing YTD Net Actual - end
			//	processing This Year Buget Gross - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_This_Year_Buget_Gross->afterSuccessfulSave();
				}
	//	processing This Year Buget Gross - end
			//	processing This Year Budget Net - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_This_Year_Budget_Net->afterSuccessfulSave();
				}
	//	processing This Year Budget Net - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: actuals_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: actuals_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["YTD Net Actual"] = $evalues["YTD Net Actual"];
	$data["This Year Buget Gross"] = $evalues["This Year Buget Gross"];
	$data["This Year Budget Net"] = $evalues["This Year Budget Net"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("YTD Net Actual"))
		$xt->assign("YTD_Net_Actual_fieldblock",true);
	else
		$xt->assign("YTD_Net_Actual_tabfieldblock",true);
	$xt->assign("YTD_Net_Actual_label",true);
	if(isEnableSection508())
		$xt->assign_section("YTD_Net_Actual_label","<label for=\"".GetInputElementId("YTD Net Actual", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("This Year Buget Gross"))
		$xt->assign("This_Year_Buget_Gross_fieldblock",true);
	else
		$xt->assign("This_Year_Buget_Gross_tabfieldblock",true);
	$xt->assign("This_Year_Buget_Gross_label",true);
	if(isEnableSection508())
		$xt->assign_section("This_Year_Buget_Gross_label","<label for=\"".GetInputElementId("This Year Buget Gross", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("This Year Budget Net"))
		$xt->assign("This_Year_Budget_Net_fieldblock",true);
	else
		$xt->assign("This_Year_Budget_Net_tabfieldblock",true);
	$xt->assign("This_Year_Budget_Net_label",true);
	if(isEnableSection508())
		$xt->assign_section("This_Year_Budget_Net_label","<label for=\"".GetInputElementId("This Year Budget Net", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='actuals_view.php?".$viewlink."'\"");
