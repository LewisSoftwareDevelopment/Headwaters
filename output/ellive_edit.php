...
$layout->blocks["top"][] = "details";$page_layouts["ellive_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "ellive_inline_edit.htm" : "ellive_edit.htm";
...
//	processing Aug - end
//	processing Sep - begin
	$condition = 1;

	if($condition)
	{
		$control_Sep = $pageObject->getControl("Sep", $id);
		$control_Sep->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Dec - end
	...
	//	processing Aug - end
			//	processing Sep - begin
				...
				if($condition)
				{
					$control_Sep->afterSuccessfulSave();
				}
	//	processing Sep - end
			//	processing Oct - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: ellive_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: ellive_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("Sep_label",true);
	if(isEnableSection508())
		$xt->assign_section("Sep_label","<label for=\"".GetInputElementId("Sep", $id, PAGE_EDIT)."\">","</label>");
...
//	Sep - 
	...
	$showValues["Sep"] = $value;
	$showFields[] = "Sep";
		$showRawValues["Sep"] = substr($data["Sep"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='ellive_view.php?".$viewlink."'\"");
