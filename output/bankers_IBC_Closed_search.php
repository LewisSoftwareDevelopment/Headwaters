...
include("include/bankers_IBC_Closed_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["bankers_IBC_Closed_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "bankers_IBC_Closed_search.htm";
...
$params['shortTableName'] = 'bankers_IBC_Closed';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Rows"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Rows")] = GetTableURL($pageObject->pSet->getLookupTable("Rows"));
	
	$pageObject->fillFieldToolTips("Rows");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Rows");
	...
	if(isEnableSection508())
		$xt->assign_section("Rows_label","<label for=\"".GetInputElementId("Rows", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Rows_label", true);
	
	$xt->assign("Rows_fieldblock", true);
	$xt->assignbyref("Rows_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Rows_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Rows_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Rows", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Rows");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Rows", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Rows", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
