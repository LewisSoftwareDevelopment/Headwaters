...
include("include/referral_individual_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["referral_individual_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "referral_individual_inline_edit.htm" : "referral_individual_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: referral_individual_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: referral_individual_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "referral individual";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='referral_individual_view.php?".$viewlink."'\"");
