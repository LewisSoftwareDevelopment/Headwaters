...
add_nocache_headers();
include("include/referral_company_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["referral_company_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "referral_company_inline_add.htm";
else
	$templatefile = "referral_company_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: referral_company_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'referral_company_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'referral_company_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: referral_company_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "referral company";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="company" && postvalue("field")=="Referral_Source_Company") 
	{
		$lookupFieldName = "Referral Source-Company";
		$lookupTable = "referral company";
		$linkFieldName = "Name";
		$dispfield = "Name";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Referral Source-Company");
