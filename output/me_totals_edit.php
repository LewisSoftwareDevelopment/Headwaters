...
$layout->blocks["top"][] = "details";$page_layouts["me_totals_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "me_totals_inline_edit.htm" : "me_totals_edit.htm";
...
//	processing ELClient - begin
	$condition = 1;

	if($condition)
	{
		$control_ELClient = $pageObject->getControl("ELClient", $id);
		$control_ELClient->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

	}
//	processing ELClient - end
...
//	processing ELSlot - end
	...
	//	processing DSlot - end
			//	processing ELClient - begin
				...
				if($condition)
				{
					$control_ELClient->afterSuccessfulSave();
				}
	//	processing ELClient - end
			//	processing ELDealType - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: me_totals_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: me_totals_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["ELClient"] = $evalues["ELClient"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("ELClient"))
		$xt->assign("ELClient_fieldblock",true);
	else
		$xt->assign("ELClient_tabfieldblock",true);
	$xt->assign("ELClient_label",true);
	if(isEnableSection508())
		$xt->assign_section("ELClient_label","<label for=\"".GetInputElementId("ELClient", $id, PAGE_EDIT)."\">","</label>");
...
//	ELClient - 
	$value = $pageObject->showDBValue("ELClient", $data, $keylink);
	$showValues["ELClient"] = $value;
	$showFields[] = "ELClient";
		$showRawValues["ELClient"] = substr($data["ELClient"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='me_totals_view.php?".$viewlink."'\"");
