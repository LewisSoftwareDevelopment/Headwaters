...
$layout->blocks["top"][] = "details";$page_layouts["nda_per_month_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "nda_per_month_inline_edit.htm" : "nda_per_month_edit.htm";
...
//	processing Date - end
//	processing Total per Month - begin
	$condition = 1;

	if($condition)
	{
		$control_Total_per_Month = $pageObject->getControl("Total per Month", $id);
		$control_Total_per_Month->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Total per Month - end
	...
	//	processing Date - end
			//	processing Total per Month - begin
				...
				if($condition)
				{
					$control_Total_per_Month->afterSuccessfulSave();
				}
	//	processing Total per Month - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: nda_per_month_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: nda_per_month_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Total per Month"] = $evalues["Total per Month"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Total per Month"))
		$xt->assign("Total_per_Month_fieldblock",true);
	else
		$xt->assign("Total_per_Month_tabfieldblock",true);
	$xt->assign("Total_per_Month_label",true);
	if(isEnableSection508())
		$xt->assign_section("Total_per_Month_label","<label for=\"".GetInputElementId("Total per Month", $id, PAGE_EDIT)."\">","</label>");
...
//	Total per Month - 
	$value = $pageObject->showDBValue("Total per Month", $data, $keylink);
	$showValues["Total per Month"] = $value;
	$showFields[] = "Total per Month";
		$showRawValues["Total per Month"] = substr($data["Total per Month"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "nda per month";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='nda_per_month_view.php?".$viewlink."'\"");
