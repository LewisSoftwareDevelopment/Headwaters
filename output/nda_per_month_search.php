...
include("include/nda_per_month_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["nda_per_month_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "nda_per_month_search.htm";
...
$params['shortTableName'] = 'nda_per_month';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Total per Month"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Total per Month")] = GetTableURL($pageObject->pSet->getLookupTable("Total per Month"));
	
	$pageObject->fillFieldToolTips("Total per Month");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Total per Month");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Total per Month";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Total_per_Month_label","<label for=\"".GetInputElementId("Total per Month", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Total_per_Month_label", true);
	
	$xt->assign("Total_per_Month_fieldblock", true);
	$xt->assignbyref("Total_per_Month_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Total_per_Month_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Total_per_Month_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Total_per_Month", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Total per Month");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Total per Month", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Total per Month", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
