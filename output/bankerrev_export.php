...
$layout->blocks["top"][] = "export";$page_layouts["bankerrev_export"] = $layout;
...
$xt->display("bankerrev_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=bankerrev.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankerrev.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankerrev.xml");
	...
	{
		$values = array();
			$values["Year"] = $pageObject->showDBValue("Year", $row);
			$values["Revenue total"] = $pageObject->showDBValue("Revenue total", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bankerrev.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["Year"] = $pageObject->getViewControl("Year")->showDBValue($row, "");
			$values["Revenue total"] = $row["Revenue total"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Year"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Revenue total"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Revenue Total").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["Year"] = $pageObject->getViewControl("Year")->showDBValue($row, "");
					$values["Revenue total"] = $pageObject->getViewControl("Revenue total")->showDBValue($row, "");
					$values["Name"] = $pageObject->getViewControl("Name")->showDBValue($row, "");
					$values["ID"] = $pageObject->getViewControl("ID")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Year"]);
					else
						echo $values["Year"];
			echo '</td>';
							echo '<td>';
			
									echo $values["Revenue total"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
