...
include("include/Live_Sell_Side_Engagements_by_Enterprise_Value_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["Live_Sell_Side_Engagements_by_Enterprise_Value_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Live_Sell_Side_Engagements_by_Enterprise_Value_search.htm";
...
$params['shortTableName'] = 'Live_Sell_Side_Engagements_by_Enterprise_Value';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Vertical"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Vertical")] = GetTableURL($pageObject->pSet->getLookupTable("Vertical"));
	
	$pageObject->fillFieldToolTips("Vertical");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Vertical");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Vertical";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Vertical_label","<label for=\"".GetInputElementId("Vertical", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Vertical_label", true);
	
	$xt->assign("Vertical_fieldblock", true);
	$xt->assignbyref("Vertical_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Vertical_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Vertical_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Vertical", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Vertical");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Vertical", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Vertical", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
