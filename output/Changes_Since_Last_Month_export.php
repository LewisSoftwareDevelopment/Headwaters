...
$layout->blocks["top"][] = "export";$page_layouts["Changes_Since_Last_Month_export"] = $layout;
...
$xt->display("Changes_Since_Last_Month_export.htm");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Changes_Since_Last_Month.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["ELClient"] = $pageObject->showDBValue("ELClient", $row);
			...
			$values["Deed"] = $pageObject->showDBValue("Deed", $row);
			$values["EL"] = $pageObject->showDBValue("EL", $row);
			$values["Live"] = $pageObject->showDBValue("Live", $row);
			$values["LM"] = $pageObject->showDBValue("LM", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Changes_Since_Last_Month.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["ELClient"] = $pageObject->getViewControl("ELClient")->showDBValue($row, "");
			...
			$values["Deed"] = $pageObject->getViewControl("Deed")->showDBValue($row, "");
			$values["EL"] = $pageObject->getViewControl("EL")->showDBValue($row, "");
			$values["Live"] = $pageObject->getViewControl("Live")->showDBValue($row, "");
			$values["LM"] = $pageObject->getViewControl("LM")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ELClient"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Deed"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("ELClient").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Deed").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("EL").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Live").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("LM").'</td>';	
	}
	else
	{
		echo "<td>"."CClient"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["ELClient"]);
					else
						echo $values["ELClient"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
