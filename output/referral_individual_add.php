...
$layout->blocks["top"][] = "details";$page_layouts["referral_individual_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "referral_individual_inline_add.htm";
else
	$templatefile = "referral_individual_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: referral_individual_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'referral_individual_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'referral_individual_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: referral_individual_".$pageObject->getPageType().".php");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="company" && postvalue("field")=="Referral_Scource_Ind_") 
	{
		$lookupFieldName = "Referral Scource-Ind.";
		$lookupTable = "referral individual";
		$linkFieldName = "Name";
		$dispfield = "Name";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "company", "Referral Scource-Ind.");
