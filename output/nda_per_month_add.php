...
$layout->blocks["top"][] = "details";$page_layouts["nda_per_month_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "nda_per_month_inline_add.htm";
else
	$templatefile = "nda_per_month_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: nda_per_month_add.php');
...
//	processing Date - end
//	processing Total per Month - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Total_per_Month = $pageObject->getControl("Total per Month", $id);
		$control_Total_per_Month->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'nda_per_month_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'nda_per_month_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: nda_per_month_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["Total per Month"]=@$avalues["Total per Month"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Total_per_Month_label","<label for=\"".GetInputElementId("Total per Month", $id, PAGE_ADD)."\">","</label>");
