...
$layout->blocks["top"][] = "details";$page_layouts["bankerrev_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "bankerrev_inline_add.htm";
else
	$templatefile = "bankerrev_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: bankerrev_add.php');
...
//	processing Year - start
	...
	if($inlineAddOption)
	{
		...
		$control_Year->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Year - end
//	processing Revenue total - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Revenue_total = $pageObject->getControl("Revenue total", $id);
		$control_Revenue_total->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="bankers")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'bankerrev_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'bankerrev_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: bankerrev_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["Revenue total"]=@$avalues["Revenue total"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Year_label","<label for=\"".GetInputElementId("Year", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Revenue_total_label","<label for=\"".GetInputElementId("Revenue total", $id, PAGE_ADD)."\">","</label>");
