...
$layout->blocks["top"][] = "search";$page_layouts["changes_last_month_end_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "changes_last_month_end_search.htm";
...
$params['shortTableName'] = 'changes_last_month_end';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("LMLive"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("LMLive")] = GetTableURL($pageObject->pSet->getLookupTable("LMLive"));
	
	$pageObject->fillFieldToolTips("LMLive");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("LMLive");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("LMLive_label","<label for=\"".GetInputElementId("LMLive", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("LMLive_label", true);
	
	$xt->assign("LMLive_fieldblock", true);
	$xt->assignbyref("LMLive_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("LMLive_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("LMLive_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_LMLive", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("LMLive");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"LMLive", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"LMLive", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("Dead"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Dead")] = GetTableURL($pageObject->pSet->getLookupTable("Dead"));
	
	$pageObject->fillFieldToolTips("Dead");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Dead");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Dead_label","<label for=\"".GetInputElementId("Dead", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Dead_label", true);
	
	$xt->assign("Dead_fieldblock", true);
	$xt->assignbyref("Dead_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Dead_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Dead_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Dead", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Dead");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Dead", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Dead", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("NewEl"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("NewEl")] = GetTableURL($pageObject->pSet->getLookupTable("NewEl"));
	
	$pageObject->fillFieldToolTips("NewEl");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("NewEl");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("NewEl_label","<label for=\"".GetInputElementId("NewEl", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("NewEl_label", true);
	
	$xt->assign("NewEl_fieldblock", true);
	$xt->assignbyref("NewEl_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("NewEl_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("NewEl_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_NewEl", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("NewEl");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"NewEl", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"NewEl", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("CurrentLive"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("CurrentLive")] = GetTableURL($pageObject->pSet->getLookupTable("CurrentLive"));
	
	$pageObject->fillFieldToolTips("CurrentLive");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("CurrentLive");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "CurrentLive";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("CurrentLive_label","<label for=\"".GetInputElementId("CurrentLive", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("CurrentLive_label", true);
	
	$xt->assign("CurrentLive_fieldblock", true);
	$xt->assignbyref("CurrentLive_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("CurrentLive_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("CurrentLive_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_CurrentLive", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("CurrentLive");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CurrentLive", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CurrentLive", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("ELClinet"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("ELClinet")] = GetTableURL($pageObject->pSet->getLookupTable("ELClinet"));
	
	$pageObject->fillFieldToolTips("ELClinet");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("ELClinet");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "ELClinet";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("ELClinet_label","<label for=\"".GetInputElementId("ELClinet", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("ELClinet_label", true);
	
	$xt->assign("ELClinet_fieldblock", true);
	$xt->assignbyref("ELClinet_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("ELClinet_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("ELClinet_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_ELClinet", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("ELClinet");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ELClinet", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"ELClinet", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
