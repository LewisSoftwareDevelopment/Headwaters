...
$layout->blocks["top"][] = "export";$page_layouts["Live_Sell_Side_Engagements_by_Enterprise_Value_export"] = $layout;
...
$xt->display("Live_Sell_Side_Engagements_by_Enterprise_Value_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Live_Sell_Side_Engagements_by_Enterprise_Value.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Live_Sell_Side_Engagements_by_Enterprise_Value.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Live_Sell_Side_Engagements_by_Enterprise_Value.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Vertical"] = $pageObject->showDBValue("Vertical", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Live_Sell_Side_Engagements_by_Enterprise_Value.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Vertical"] = $pageObject->getViewControl("Vertical")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Vertical"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Vertical").'</td>';	
	}
	else
	{
		echo "<td>"."Company"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Vertical"]);
					else
						echo $values["Vertical"];
			echo '</td>';
			echo "</tr>";
		}
