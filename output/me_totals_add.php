...
$layout->blocks["top"][] = "details";$page_layouts["me_totals_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "me_totals_inline_add.htm";
else
	$templatefile = "me_totals_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: me_totals_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'me_totals_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'me_totals_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: me_totals_".$pageObject->getPageType().".php");
