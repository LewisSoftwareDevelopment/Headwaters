...
include("include/bankerrev_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["bankerrev_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "bankerrev_inline_edit.htm" : "bankerrev_edit.htm";
...
//	processing Year - begin
	$condition = 1;

	if($condition)
	{
		$control_Year = $pageObject->getControl("Year", $id);
		$control_Year->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Year - end
//	processing Revenue total - begin
	$condition = 1;

	if($condition)
	{
		$control_Revenue_total = $pageObject->getControl("Revenue total", $id);
		$control_Revenue_total->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Revenue total - end
	...
	//	do event
		...
		{		
			...
			//	processing Year - begin
				...
				if($condition)
				{
					$control_Year->afterSuccessfulSave();
				}
	//	processing Year - end
			//	processing Revenue total - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_Revenue_total->afterSuccessfulSave();
				}
	//	processing Revenue total - end
			//	processing Name - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: bankerrev_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: bankerrev_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["Revenue total"] = $evalues["Revenue total"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Year_label","<label for=\"".GetInputElementId("Year", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Revenue total"))
		$xt->assign("Revenue_total_fieldblock",true);
	else
		$xt->assign("Revenue_total_tabfieldblock",true);
	$xt->assign("Revenue_total_label",true);
	if(isEnableSection508())
		$xt->assign_section("Revenue_total_label","<label for=\"".GetInputElementId("Revenue total", $id, PAGE_EDIT)."\">","</label>");
...
//	Revenue total - Number
	$value = $pageObject->showDBValue("Revenue total", $data, $keylink);
	$showValues["Revenue total"] = $value;
	$showFields[] = "Revenue total";
		$showRawValues["Revenue total"] = substr($data["Revenue total"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "bankerrev";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "bankerrev";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='bankerrev_view.php?".$viewlink."'\"");
