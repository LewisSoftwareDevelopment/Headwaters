...
$layout->blocks["top"][] = "search";$page_layouts["inmarketloi_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "inmarketloi_search.htm";
...
$params['shortTableName'] = 'inmarketloi';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Sep"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Sep")] = GetTableURL($pageObject->pSet->getLookupTable("Sep"));
	
	$pageObject->fillFieldToolTips("Sep");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Sep");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Sep_label","<label for=\"".GetInputElementId("Sep", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Sep_label", true);
	
	$xt->assign("Sep_fieldblock", true);
	$xt->assignbyref("Sep_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Sep_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Sep_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Sep", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Sep");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Sep", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Sep", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
