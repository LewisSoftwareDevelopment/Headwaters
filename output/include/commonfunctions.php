...
function checkTableName($shortTName, $type=false)
{
	...
	if ("company" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("bankers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("bankers_engagements_YTD" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("bankerrev" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("bankerrank" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("fee_split" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("referral_company" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("referral_individual" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("bankers_Combo" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("bankers_Report3" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("bankerrev_years" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("company_Report" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("bankers_IBC_Closed" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("All" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("bprevstore" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("bpengagements" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Banker_Productivity" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("bpengagmentstotal" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("bankers_engagement_totals" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Live_Engagements_by_Deal_Slot" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Live_Sell_Side_Engagements_by_Enterprise_Value" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("YTD_Engagements" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Live_Engagements_by_Status" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Referral_Utilization" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Snapshot" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Changes_Since_Last_Month_EL" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Changes_Since_Last_Month_Dead" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("Changes_Since_Last_Month_Closed" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("changes_last_month_end" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("bankerrank_Chart" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("me_el" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("me_dead" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("me_close" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Changes_Since_Last_Month" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("me_totals" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("nda_per_month" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("utilization_targets" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Revenue_Estimates1" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("actuals" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ellive" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("inmarketloi" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ssvsop" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Trend_Analysis_LTM" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	$arr = array();
		$arr[]="company";
		$arr[]="bankers";
		$arr[]="bankers engagements YTD";
		$arr[]="bankerrev";
		$arr[]="bankerrank";
		$arr[]="fee split";
		$arr[]="referral company";
		$arr[]="referral individual";
		$arr[]="bankers Combo";
		$arr[]="bankers Report3";
		$arr[]="bankerrev years";
		$arr[]="company Report";
		$arr[]="bankers IBC/Closed";
		$arr[]="All";
		$arr[]="bprevstore";
		$arr[]="bpengagements";
		$arr[]="Banker Productivity";
		$arr[]="bpengagmentstotal";
		$arr[]="bankers engagement totals";
		$arr[]="Live Engagements by Deal Slot";
		$arr[]="Live Sell-Side Engagements by Enterprise Value";
		$arr[]="YTD Engagements";
		$arr[]="Live Engagements by Status";
		$arr[]="Referral Utilization";
		$arr[]="Snapshot";
		$arr[]="Changes Since Last Month-EL";
		$arr[]="Changes Since Last Month-Dead";
		$arr[]="Changes Since Last Month-Closed";
		$arr[]="changes last month end";
		$arr[]="bankerrank Chart";
		$arr[]="me-el";
		$arr[]="me-dead";
		$arr[]="me-close";
		$arr[]="Changes Since Last Month";
		$arr[]="me-totals";
		$arr[]="nda per month";
		$arr[]="utilization targets";
		$arr[]="Revenue Estimates1";
		$arr[]="actuals";
		$arr[]="ellive";
		$arr[]="inmarketloi";
		$arr[]="ssvsop";
		$arr[]="Trend Analysis LTM";
	return $arr;
}
...
{
	if($shorttable=="bankerrank_Chart")
		return "2DColumn";
	return "";
}
