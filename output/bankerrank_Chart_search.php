...
include("include/bankerrank_Chart_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["bankerrank_Chart_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "bankerrank_Chart_search.htm";
...
$params['shortTableName'] = 'bankerrank_Chart';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Rank"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Rank")] = GetTableURL($pageObject->pSet->getLookupTable("Rank"));
	
	$pageObject->fillFieldToolTips("Rank");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Rank");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Rank_label","<label for=\"".GetInputElementId("Rank", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Rank_label", true);
	
	$xt->assign("Rank_fieldblock", true);
	$xt->assignbyref("Rank_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Rank_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Rank_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Rank", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Rank");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Rank", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Rank", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
