...
$layout->blocks["top"][] = "search";$page_layouts["bankerrev_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "bankerrev_search.htm";
...
$params['shortTableName'] = 'bankerrev';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Revenue total"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Revenue total")] = GetTableURL($pageObject->pSet->getLookupTable("Revenue total"));
	
	$pageObject->fillFieldToolTips("Revenue total");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Revenue total");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Revenue total";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Revenue_total_label","<label for=\"".GetInputElementId("Revenue total", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Revenue_total_label", true);
	
	$xt->assign("Revenue_total_fieldblock", true);
	$xt->assignbyref("Revenue_total_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Revenue_total_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Revenue_total_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Revenue_total", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Revenue total");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Revenue total", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Revenue total", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
